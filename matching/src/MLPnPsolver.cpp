    ...
    while (mnIterations < mRansacMaxIts || nCurrentIterations < nIterations)
    {
        ...
        if (mnInliersi >= mRansacMinInliers)
        {
            ...
            if (mnInliersi > mnBestInliers)
            {
                ...
                mBestTcw.block<3, 3>(0, 0) = mRi.cast<float>();
                mBestTcw.block<3, 1>(0, 3) = mti.cast<float>();
            }
            if (Refine())
            {
...
void MLPnPsolver::CheckInliers()
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        Eigen::Vector2f uv = mpCamera->project(Eigen::Vector3f(xc, yc, zc));

        float distX = P2D.x - uv[0];
        float distY = P2D.y - uv[1];

        float error2 = distX * distX + distY * distY;

        if (error2 < mfMaxError)
        {
            mvbInliersi[i] = true;
            mnInliersi++;
        }
...
bool MLPnPsolver::Refine()
{
    ...
    if (mnInliersi > mRansacMinInliers)
    {
        ...
        mRefinedTcw.block<3, 3>(0, 0) = mRi.cast<float>();
        mRefinedTcw.block<3, 1>(0, 3) = mti.cast<float>();
        return true;
    }
    return false;
}
