...
void MapPoint::Replace(MapPoint* pMP)
{
...

    {
        ...
        // {
        //     MapPoint *pMP_o = pME->theOtherPt(this);
        //     MapEdge* pME_o = pMP->getEdge(pMP_o);
        //     if(!pME_o || pME_o->isBad())
        //         continue;
        //     auto obs = pME->getObservations();
        //     for(auto ob : obs)
        //     {
        //         pME_o->addObservation(ob.first, ob.second);
        //         ob.first->AddMapEdge(pME_o, ob.second);
        //     }
        //     pME->mbBad = true;
        //     pME_o->checkValid();
        // }
...
MapColine* MapPoint::addColine(MapPoint* pMPs, MapPoint* pMPe, KeyFrame* pKF, float weight)
{
    ...
    if(distRatio < 0.2 || distRatio > 5 || std::fabs(viewDegen1) > 0.996 || std::fabs(viewDegen2) > 0.996)
        return nullptr;
    if(weight < 0)
    {
