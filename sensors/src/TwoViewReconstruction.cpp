...
float TwoViewReconstruction::CheckHomography(const Eigen::Matrix3f &H21, const Eigen::Matrix3f &H12, vector<bool> &vbMatchesInliers, float sigma)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const KeyPointEx &kp1 = mvKeys1[mvMatches12[i].first];
        const KeyPointEx &kp2 = mvKeys2[mvMatches12[i].second];

        const float u1 = kp1.mPos[0];
        const float v1 = kp1.mPos[1];
        const float u2 = kp2.mPos[0];
        const float v2 = kp2.mPos[1];
...
void TwoViewReconstruction::Normalize(const vector<KeyPointEx> &vKeys, vector<cv::Point2f> &vNormalizedPoints, Eigen::Matrix3f &T)
{
    ...
    for(int i=0; i<N; i++)
    {
        meanX += vKeys[i].mPos[0];
        meanY += vKeys[i].mPos[1];
    }
    ...
    for(int i=0; i<N; i++)
    {
        vNormalizedPoints[i].x = vKeys[i].mPos[0] - meanX;
        vNormalizedPoints[i].y = vKeys[i].mPos[1] - meanY;
...
int TwoViewReconstruction::CheckRT(const Eigen::Matrix3f &R, const Eigen::Vector3f &t, const vector<KeyPointEx> &vKeys1, const vector<KeyPointEx> &vKeys2,
                                    ...
                                    const Eigen::Matrix3f &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        const KeyPointEx &kp1 = vKeys1[vMatches12[i].first];
        const KeyPointEx &kp2 = vKeys2[vMatches12[i].second];

        Eigen::Vector3f p3dC1;
        Eigen::Matrix4f A;
        A.block<1,4>(0,0) = kp1.mPos[0] * P1.block<1,4>(2,0) - P1.block<1,4>(0,0);
        A.block<1,4>(1,0) = kp1.mPos[1] * P1.block<1,4>(2,0) - P1.block<1,4>(1,0);
        A.block<1,4>(2,0) = kp2.mPos[0] * P2.block<1,4>(2,0) - P2.block<1,4>(0,0);
        A.block<1,4>(3,0) = kp2.mPos[1] * P2.block<1,4>(2,0) - P2.block<1,4>(1,0);
        Eigen::JacobiSVD<Eigen::Matrix4f> svd(A, Eigen::ComputeFullV);
        Eigen::Vector4f x3Dh = svd.matrixV().col(3);
        if(x3Dh(3)==0)
            continue;
        // Euclidean coordinates
        p3dC1 = x3Dh.head(3)/x3Dh(3);
        ...
        float squareError1 = (im1x-kp1.mPos[0])*(im1x-kp1.mPos[0])+(im1y-kp1.mPos[1])*(im1y-kp1.mPos[1]);
        ...
        float squareError2 = (im2x-kp2.mPos[0])*(im2x-kp2.mPos[0])+(im2y-kp2.mPos[1])*(im2y-kp2.mPos[1]);
